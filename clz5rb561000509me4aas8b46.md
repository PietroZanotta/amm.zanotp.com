...
$$A \ket{\psi}_n= \sum_{i=0}^{2^n-1} \sqrt{(1 -f(S_i)} \sqrt{p_i}\ket{S_i}n\ket{0} + \sum{i=0}^{2^n-1} \sqrt{f(S_i)} \sqrt{p_i}\ket{S_i}_n\ket{1}$$
...
$$C\ket\psi _n\ket 0 = \ket \phi_n= \sum_{i<K} \sqrt{p_i}\ket i_n\otimes \ket 0 + \sum_{i\geq K} \sqrt{p_i}\ket i_n \otimes \ket 1$$
...
$$P_1\approx\sum_{i<K} p_i(\frac 12-c) + \sum_{i\geq K} p_i(\frac 12-c + \frac{2c (i-K)}{2^n-1 - K})=\frac 12-c + \frac{2c }{2^n-1 - K} \sum_{i \geq K} p_i(i-K)$$
